...
void setup()
{
	...
	p1 = glm::vec3(0.0f, 0.4f, 0.0f);
	p2 = glm::vec3(-0.2f, 0.0f, 0.0f);
	p3 = glm::vec3(0.0f, 0.0f, 0.2f);
	p4 = glm::vec3(0.2f, 0.0f, 0.0f);
	...
	p1 = glm::vec3(0.2f, 0.4f, 0.0f);
	p2 = glm::vec3(-0.2f, 0.2f, 0.0f);
	p3 = glm::vec3(-0.2f, 0.0f, 0.2f);
	p4 = glm::vec3(0.2f, 0.0f, 0.0f);
	...
	// Push the vertices for each face in counte clockwise manner
	vertices.push_back(VertexFormat(p1, glm::vec4(1.0f, 0.0f, 0.0f, 0.0f)));
	vertices.push_back(VertexFormat(p2, glm::vec4(1.0f, 0.0f, 0.0f, 0.0f)));
	vertices.push_back(VertexFormat(p3, glm::vec4(1.0f, 0.0f, 0.0f, 0.0f)));
															  
	vertices.push_back(VertexFormat(p1, glm::vec4(1.0f, 0.0f, 0.0f, 0.0f)));
	vertices.push_back(VertexFormat(p3, glm::vec4(1.0f, 0.0f, 0.0f, 0.0f)));
	vertices.push_back(VertexFormat(p4, glm::vec4(1.0f, 0.0f, 0.0f, 0.0f)));
															  
	vertices.push_back(VertexFormat(p1, glm::vec4(1.0f, 0.0f, 0.0f, 0.0f)));
	vertices.push_back(VertexFormat(p5, glm::vec4(1.0f, 0.0f, 0.0f, 0.0f)));
	vertices.push_back(VertexFormat(p2, glm::vec4(1.0f, 0.0f, 0.0f, 0.0f)));
...
bool returnMTV(ConvexHull &A, ConvexHull &B, glm::vec3 &mtv)
{
	...
	for (it = A.faceNormalSet.begin(); it != A.faceNormalSet.end(); it++)
	{
		...
		{
			temp = glm::dot(n, glm::vec3(transform1 * glm::vec4(*xy, 1.0f)));
			if (min1 > temp)
				min1 = temp;
			if (max1 < temp)
				max1 = temp;
		...
		//find the dot produnct of each vertex in world space on the axis
		for (xy = B.vertexSet.begin(); xy != B.vertexSet.end(); xy++)
		{
			temp = glm::dot(n, glm::vec3(transform2 * glm::vec4(*xy, 1.0f)));
			if (min2 > temp)
				min2 = temp;
			if (max2 < temp)
				max2 = temp;
